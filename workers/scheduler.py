...
from workers import run_analyze, run_match, run_notify, run_scrape
...
def _stages(
    sources: Iterable[JobSource], notifier_factory: Callable[[str], Notifier]
) -> tuple[Stage, ...]:
...
def run_pipeline(
    ...
    """Run scrape, analyze, match, and notify in sequence.
    ...
    """
    for name, stage in _stages(sources, notifier_factory):
        try:
